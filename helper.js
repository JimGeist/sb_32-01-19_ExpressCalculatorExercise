/** Express Statistical Application for mean, median, and mode calculations. */

/** Helper Functions                                                         */

function checkNumbers(inNumbers) {

  /* checks all the values in inNumbers to make sure they are indeed a number. 
     Function stops checking numbers when an error occurs.

     When all values in inNumbers are numbers, function returns:
      {
        "numbersAreValid": true,
        "validatedNumbers": [array of inNumbers as an array of integers]
      }
     When a non-number is encountered, function returns:
      {
        "numbersAreValid": false,
        "message" = 'nums=${inNumbers}' is invalid. '${inNums[idx]}' is not a number.`
      }
  ...
  */
  ...
  for (let idx = 0; idx < inNums.length; idx++) {
    ...
    if (isNaN(nbrText)) {
      ...
      results["message"] = `'nums=${inNumbers}' is invalid. '${inNums[idx].trim()}' is not a number.`;
      return results;
    }
...
function calcMean(numbers) {
  ...
  numbers.forEach(number => { mean = mean + number; })
